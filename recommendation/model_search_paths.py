...
class Op(nn.Module):
    ...
    def forward(self, x, adjs, ws):
        ...
        return sum(ws[i] * torch.spmm(adjs[i], x) for i in range(num_op) if i in idx) / num
...
class Cell(nn.Module):
    ...
    '''
    def __init__(self, n_step, n_hid_prev, n_hid, cstr, k, use_norm = True, use_nl = True):
...
class Model_paths(nn.Module):
    ...
    def __init__(self, in_dims, n_hid, n_adjs, n_steps, cstr, lambda_seq, lambda_res, k, attn_dim = 64, use_norm = True, out_nl = True):
        ...
        for i in range(len(n_steps)):
            self.metas.append(Cell(n_steps[i], n_hid, n_hid, cstr, k, use_norm = use_norm, use_nl = out_nl))
    ...
    def forward(self, node_feats, node_types, adjs):
        ...
        for i, meta in enumerate(self.metas):
            ...
            hidi = meta(hid, adjs, ws_seq, ws_res)
            temps.append(hidi)
    ...
    def getid(self, seq_res, lam):
        seq_softmax = F.softmax(seq_res, dim=-1)
        ...
        threshold = lam * max + (1 - lam) * min
        return [[k for k in range(length) if seq_softmax[j][k].item()>=threshold[j]] for j in range(len(seq_softmax))]
    ...
    def sample_final(self, eps):
        ...
        if np.random.uniform() < eps:
            for i in range(len(self.metas)): # len of self.metas is 1
                temp = []
            ...
            for i in range(len(self.metas)):
                temp = []
                temp.append(None if self.as_res[i] is None else torch.randint(low=0, high=self.as_res[i].size(-1), size=self.as_res[i].size()[:-1]).cuda())  # self.as_res[0]: shape [3,6]   high:  6   size :3
                temp.append(None if self.as_last_res[i] is None else torch.randint(low=0, high=self.as_last_res[i].size(-1), size=self.as_last_res[i].size()[:-1]).cuda()) # self.as_last_res[0]: shape [3,3]   high:  3   size :3
    ...
    def parse(self):
        ...
        for i in range(len(idxes_seq)):
            ...
            msg_seq.append(
                map_seq if idxes_seq[i][0] is None else idxes_seq[i][0] + map_seq)  # idxes_seq[0][0]+idxes_seq[0][1]
