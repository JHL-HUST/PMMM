...
parser.add_argument('--k', type=int, default=2, help='sampling')
...
def main():
    ...
    model_s = Model_paths(in_dims, args.n_hid, len(adjs_pt), args.steps_s, cstr_source[args.dataset], args.lam_seq, args.lam_res, args.k).cuda()
    model_t = Model_paths(in_dims, args.n_hid, len(adjs_pt), args.steps_t, cstr_target[args.dataset], args.lam_seq, args.lam_res, args.k).cuda()
...
def train(node_feats, node_types, adjs, pos_train, neg_train, pos_val, neg_val, model_s, model_t, optimizer_w, optimizer_a, eps):
    ...
    out_s = model_s(node_feats, node_types, adjs)
    out_t = model_t(node_feats, node_types, adjs)
