...
from model_search_paths import Model_paths
...
parser.add_argument('--epochs', type=int, default=30, help='number of epochs for supernet training')
...
parser.add_argument('--lam_seq', type=float, default=1, help='threshold')
parser.add_argument('--lam_res', type=float, default=1, help='threshold')
...
logdir = os.path.join("log/search", args.dataset+'2')
...
def main():
    ...
    with open(os.path.join(prefix, "node_features.pkl"), "rb") as f:
        ...
        f.close()
    node_feats = torch.from_numpy(node_feats.astype(np.float32)).cuda()# is a numpy array whose shape is (num_of_nodes, num_of_features)

    node_types = np.load(os.path.join(prefix, "node_types.npy")) #contains node type information and has a shape of (num_of_nodes,)
    ...
    model = Model_paths(node_feats.size(1), args.n_hid, num_node_types, len(adjs_pt), n_classes, args.steps, args.ratio,
                      cstr_nc[args.dataset], args.k, args.lam_seq, args.lam_res).cuda()
